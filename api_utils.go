 ...
 */
func CancelAllUnfinishedOrders(api SpotAPI, currencyPair CurrencyPair) int {
	if api == nil {
		log.Println("api instance is nil ??? , please new a api instance")
		return -1
	}

	orders := RE(10, api.GetPendingOrders, currencyPair)
	if orders != nil {
		c := 0
		for _, ord := range orders.([]Order) {
			_, err := api.Cancel(ord.OrderID, currencyPair)
			if err != nil {
				log.Println(err)
			}
			c++
